...
This document describes the technical architecture of the HelloFresh custom component for Home Assistant.
...
│   ├── __init__.py          # Exports HelloFreshDataUpdateCoordinator
...
│   ├── __init__.py          # Exports HelloFreshEntity
...
- `base.py` - Main coordinator class (`HelloFreshDataUpdateCoordinator`)
...
**Key class:** `HelloFreshDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
- `HelloFreshConfigFlowHandler` (main flow)
- `HelloFreshOptionsFlow` (options)
...
**Key class:** `HelloFreshEntity` (in `entity/base.py`)
...
2. `HelloFreshEntity` for common functionality
...
1. Create directory: `custom_components/hellofresh/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `HelloFreshEntity`
