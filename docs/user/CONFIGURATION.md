...
This document describes all configuration options and settings available in the HelloFresh custom integration.
...
```yaml
service: hellofresh.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: hellofresh.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
- [GitHub Issues](https://github.com/pantherale0/ha-hellofresh/issues) - Report problems
