...
- **Domain:** `hellofresh`
- **Title:** HelloFresh
- **Repository:** pantherale0/ha-hellofresh

**Key directories:**

- `custom_components/hellofresh/` - Main integration code
...
- Integration logs: `custom_components.hellofresh: debug` in `config/configuration.yaml`
...
- Use the domain `hellofresh` for all DOMAIN references
- Prefix all integration-specific classes with `HelloFresh`
- Use "HelloFresh" as the display title
...
- Inherit from platform base + `HelloFreshEntity`
...
- `tests/` mirrors `custom_components/hellofresh/` structure
